...
def interpolate(args): # pylint: disable=redefined-outer-name
    ...
    def write(output_dir, buffer):
        ...
        while item is not None:
            ...
            count += 1
            if count % 120 == 0:
                print(f"RIFE: Interpolated and saved {count} frames...")
        ...
        for f in videogen:
            ...
            if ssim < args.change:
                ...
                if model.version >= 3.9:
                    n = args.multi - 1
                    for j in range(n):
                        mid_tensor = model.inference(I0, I1, (j+1) * 1. / (n+1), args.scale)
                        mid_numpy = (mid_tensor[0] * 255.).byte().cpu().numpy().transpose(1, 2, 0)
                        buffer.put(mid_numpy[:h, :w])
                        del mid_tensor
                else:
